...
function Schemes() {
  ...
  const eligibleSchemes = schemes.filter(scheme => {
    ...
    return (
      (!criteria.maxAnnualIncome || profile?.annualIncome <= criteria.maxAnnualIncome) &&
      (!criteria.minLandSize || profile?.landSize >= criteria.minLandSize) &&
      (!criteria.maxLandSize || profile?.landSize <= criteria.maxLandSize) &&
      (!criteria.minAge || profile?.age >= criteria.minAge) &&
      (!criteria.maxAge || profile?.age <= criteria.maxAge) &&
      (!criteria.states || profile?.state && criteria.states.includes(profile.state)) &&
      (!criteria.requiredCrops || profile?.crops.some(crop => criteria.requiredCrops?.includes(crop)))
    );
  });
